...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <h1 class="font-outfit text-2xl md:text-3xl font-extrabold text-white">Catat Proyek Baru</h1>
        <p class="text-sm text-slate-400 mt-1">Lengkapi formulir di bawah untuk mencatat detail proyek Anda</p>
    </div>
    ...
    <div class="glass-panel rounded-3xl p-6 md:p-8 border border-slate-800 shadow-2xl">
        ...
        <form action="{{ route('proyek.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="nama_proyek" name="nama_proyek" required max="100" value="{{ old('nama_proyek') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="jenis_proyek" name="jenis_proyek" required
                        ...
                        <option value="Web" {{ old('jenis_proyek') == 'Web' ? 'selected' : '' }}>Website / Web Application</option>
                        <option value="Mobile" {{ old('jenis_proyek') == 'Mobile' ? 'selected' : '' }}>Mobile Application</option>
                        <option value="Desktop" {{ old('jenis_proyek') == 'Desktop' ? 'selected' : '' }}>Desktop Application</option>
                        <option value="UI Design" {{ old('jenis_proyek') == 'UI Design' ? 'selected' : '' }}>UI/UX Design</option>
                        <option value="Lainnya" {{ old('jenis_proyek') == 'Lainnya' ? 'selected' : '' }}>Kategori Lainnya</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="teknologi" name="teknologi" required max="100" value="{{ old('teknologi') }}"
                        class="block w-full px-4 py-3 bg-slate-900/50 border border-slate-700/60 rounded-xl text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all"
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4"
                    class="block w-full px-4 py-3 bg-slate-900/50 border border-slate-700/60 rounded-xl text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all"
                    placeholder="Jelaskan mengenai tujuan, fitur utama, atau detail pengerjaan proyek ini...">{{ old('deskripsi') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="date" id="tanggal_mulai" name="tanggal_mulai" required value="{{ old('tanggal_mulai') }}"
                ...
                <!-- Tanggal Selesai -->
                <div>
                    <label for="tanggal_selesai" class="block text-sm font-semibold text-slate-300 mb-2">Tanggal Selesai (Target)</label>
